...
from discriminator_model import Discriminator, QuantumBlock
from generator_model import Generator, QuantumBlock
import pennylane as qml

torch.backends.cudnn.benchmark = True

def train_fn(
    disc_D, disc_N, gen_N, gen_D, loader, opt_disc, opt_gen, l1, mse, d_scaler, g_scaler, n_qubits, n_layers
):
    D_reals = 0
    D_fakes = 0
    ...
    # Initialize quantum blocks
    quantum_block_D_D = QuantumBlock(n_qubits=n_qubits, n_layers=n_layers).to(config.DEVICE)
    quantum_block_D_N = QuantumBlock(n_qubits=n_qubits, n_layers=n_layers).to(config.DEVICE)
    quantum_block_G_N = QuantumBlock(n_qubits=n_qubits, n_layers=n_layers).to(config.DEVICE)
    quantum_block_G_D = QuantumBlock(n_qubits=n_qubits, n_layers=n_layers).to(config.DEVICE)
    ...
    for idx, (night, day) in enumerate(loop):
        ...
        with torch.cuda.amp.autocast():
            ...
            D_D_real = quantum_block_D_D(day)
            D_D_fake = quantum_block_D_D(fake_day.detach())
            ...
            D_N_real = quantum_block_D_N(night)
            D_N_fake = quantum_block_D_N(fake_night.detach())
        ...
        with torch.cuda.amp.autocast():
            D_D_fake = quantum_block_D_D(fake_day)
            D_N_fake = quantum_block_D_N(fake_night)
...
def main():
    ...
    disc_D = Discriminator(in_channels=3, use_quantum=True).to(config.DEVICE)
    disc_N = Discriminator(in_channels=3, use_quantum=True).to(config.DEVICE)
    gen_N = Generator(img_channels=3, num_residuals=9, use_quantum=True).to(config.DEVICE)
    gen_D = Generator(img_channels=3, num_residuals=9, use_quantum=True).to(config.DEVICE)
